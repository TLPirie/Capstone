...
(function() {
  ...
  window.RecommendationView = (function(superClass) {
    ...
    RecommendationView.prototype.template = '<div> <a class="tip" data-toggle="tooltip" data-placement="right"> </a> <div class="row"> <div class="col-lg-6"> <h2>Your Recommendations <a class="tip2" data-toggle="tooltip" data-placement="right" title="Here are your recommendations! They change after every new rating based on what similar users like and dislike."> <i class="icon-info-sign smallicon"></i> </a> </h2> </div> <div id="userBox" class="col-lg-5"> </div> <div class="col-lg-1"> </div> </div> <div id="container"> </div> </div>';
    ...
    RecommendationView.prototype.render = function(res) {
      var index, subjectid, subsToAdd, subsToRemove, newSubject, removeSubject;
      this.topUsersView.reRender(res);
      this.topRatedView.translateRes(res);
      subsToAdd = _.difference(res.recommendations, this.oldSubjects);
      subsToRemove = _.difference(this.oldSubjects, res.recommendations);
      this.oldSubjects = res.recommendations;
      this.$('#container').isotope('shuffle');
      for (index in subsToAdd) {
        subjectid = subsToAdd[index];
        newSubject = $('<div id="' + subjectid + '" class="element sprites ' + this.model.userObj.subLookup[subjectid].replace(/\s+/g, '').toLowerCase() + '">' + this.model.userObj.subLookup[subjectid] + '</div>');
        this.$('#container').isotope('insert', newSubject);
      }
      for (index in subsToRemove) {
        subjectid = subsToRemove[index];
        removeSubject = this.$('.' + this.model.userObj.subLookup[subjectid].replace(/\s+/g, '').toLowerCase());
        this.$('#container').isotope('remove', removeSubject);
      }
